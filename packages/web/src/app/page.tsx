...
export function AuthOverlay({ error }: { error: string }) {
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-background/95 backdrop-blur-md">
      <div className="text-center space-y-4 max-w-md p-6">
        ...
        <Button onClick={() => window.location.href = process.env.NEXT_PUBLIC_MAIN_API_URL + "/dashboard"} className="mt-4">
          Return to Dashboard
        </Button>
      </div>
    </div>
...
export default function Home() {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      ...
      try {
        ...
        setAuthError("Error verifying subscription. Please try again later.");
      }
    };
  ...
  const handle3DExport = useCallback(async (format: Export3DFormat) => {
  ...
  }, [inputTab, currentText, authToken, isExporting]);
  ...
  useEffect(() => {
    ...
    const handleDrop = (e: DragEvent) => {
      ...
      if (file && (file.type === "image/svg+xml" || file.name.endsWith(".svg"))) {
        ...
        setInputTab("file");
        setDroppedFile({ name: file.name, content: '<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100"><text x="10" y="50">Tracing image...</text></svg>' });
        fetch("/api/trace", { method: "POST", body: formData })
          .then(res => res.json())
          .then(data => {
            if (data.svg) {
              setFileSvg(data.svg);
              setDroppedFile({ name: file.name, content: data.svg });
            }
          });
      }
    };
  ...
  return (
    <main className="relative w-screen overflow-hidden bg-background h-[100svh]">
      {authError && <AuthOverlay error={authError} />}
      {/* Layer 0: 3D Canvas */}
