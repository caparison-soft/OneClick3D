...
export function DownloadDialog({ open, onOpenChange, onDownload, isExporting }: DownloadDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md">
        ...
        <Button onClick={handleDownload} disabled={isExporting} className="w-full gap-2">
          {isExporting ? (
            <div className="h-4 w-4 rounded-full border-2 border-current border-t-transparent animate-spin" />
          ) : (
            <Download className="h-4 w-4" />
          )}
          {isExporting ? "Deducting credits..." : `Download ${activeFormat.ext} (2 Credits)`}
        </Button>
      </DialogContent>
    </Dialog>
  );
